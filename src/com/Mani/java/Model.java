...
public class Model {
	...
	public List<RegBean> searchUser(String value,String searchStr) {
		...
		try
		{
			...
			{
				ps_sel = con.prepareStatement("select * from register where "+value+" like '%"+searchStr+"%"+"'"); 
				ps_sel.execute();
				rs = ps_sel.getResultSet();
				users = new ArrayList<RegBean>();
	...
	public String delUsers(String sl_no)
	{
		...
		System.out.println(sl_no);
				...
				try
					...
					return "Connection not established to DataBase";
						...
						{	
							ps_sel = con.prepareStatement("delete from register where sl_no = '"+sl_no+"'");
							ps_sel.execute();
					...
					try
						...
						return "Connection not established to DataBase";
							...
							{	
								...
								ps_ins = con.prepareStatement("insert into contacts (name, sl_no,emails,phno,dob,gender,tags)values(?,?,?,?,?,?,?)");
	...
	public List<ContactBean> listContacts(String email) {
		...
		try
		{
			...
			else
			{
				...
				ps_sel = con.prepareStatement("select * from contacts where sl_no = '"+sl_no+"'");
				ps_sel.execute();
				rs = ps_sel.getResultSet();
	...
	public List<ContactBean> searchContacts(String value,String searchStr,String email) {
		...
		try
		{
			...
			{	
				...
				ps_sel = con.prepareStatement("select * from contacts where sl_no='"+sl_no+"' AND "+value+" like '%"+searchStr+"%"+"'"); 
				ps_sel.execute();
				rs = ps_sel.getResultSet();
				users = new ArrayList<ContactBean>();
	...
	public String delContact(String sl_no)
	{
		System.out.println("Model->inside delContact()");
		//System.out.println("Model-> "+cb.getName()+" "+cb.getEmails()+" "+cb.getDob()+" "+cb.getPhno()+" "+cb.getGender());
		System.out.println(sl_no);
	...
	public List<ContactBean> bdayRmndr(String email) {
		System.out.println("inside bday() of model");
					...
					List<ContactBean> birthdayList = new ArrayList<ContactBean>();
					for(ContactBean cb:users)
					{
						...
						System.out.println("bdday "+day);
							...
							{	
								...
								int thisDay = Integer.parseInt(nday);
								int bday = Integer.parseInt(day);
								int nextMonth =(char)thisMonth+1;
								int n =thisDay-bday;
								if((thisMonth==nextMonth)&&((thisDay-n)<=7))
								{
									birthdayList.add(cb);
									break two;
								}
	...
	public String delAdminCont(String sl_no)
	{
		System.out.println("Model->inside delContact()");
		//System.out.println("Model-> "+cb.getName()+" "+cb.getEmails()+" "+cb.getDob()+" "+cb.getPhno()+" "+cb.getGender());
		System.out.println(sl_no);
